...
class SentimentClassifier:
  ...
  def __init__(self, saved_model=None):
    ...
    if saved_model is not None:
      with open(saved_model, 'rb') as savefile:
        self.model = pickle.load(savefile, encoding='latin1')
  ...
  def classify(self, tweet):
    ...
    predictions = self.classifier.predict([features])
    prediction_scores = self.classifier._predict_proba_lr([features])
    # print(prediction_scores)
    # print(predictions)
    # print(self.model['int_to_label'])
    prediction_label_scores={}
    for key in self.model['int_to_label']:
      sent_name = self.model['int_to_label'][key]
      sent_score = prediction_scores[0][key] 
      prediction_label_scores[sent_name]=round(sent_score,2)
      # print(sent_name, sent_score)
    return (self.model['int_to_label'][predictions[0]], prediction_label_scores)
  ...
  def train(self, inputdir, cache, clusters, modelout):
    ...
    for gold_line, tokenized_line in zip(gold_lines, token_lines):
      if len(gold_line.split('\t'))!=4:continue
    ...
    classifier = svm.LinearSVC(C=0.005,probability=True)
